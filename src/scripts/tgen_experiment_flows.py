...
rng: Generator = default_rng(961226)

# make a dir which will contain our results
subprocess.run([f'mkdir tgen-traces'], shell=True,
               cwd=home_dir)
...
for i in range(500):
    ...
    for j in range(1, 9):
        ...
        subprocess.run(
            [f'cp server{j}exit/tgen.1000.stdout /home/joerg_diesel_rules/traffic_shaping/tgen-traces/tgen-results/run-{i}/server{j}.tgen.stdout'], shell=True, cwd=f'{home_dir}/tornettools/tornet-0.0001/shadow.data/hosts')
