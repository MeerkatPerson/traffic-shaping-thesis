...
class ConstantMessageEmitter(MessageEmitter):
    ...
    def __init__(self, env: simpy.Environment, dir: str,
                 msg_queue: simpy.resources.store.Store, participant: str, num_streams: int, nr: int, run_nr: int, scales: Dict[str, float]) -> None:
            ...
            if self.msg_queue.items:
                ...
                if msg_type == "DATA":
                    ...
                    if self.streams_finished == self.num_streams:
                        ...
                        print(
                            f"[{self.participant}] msg emitter CONSTANT is finished, exiting!")
                        break
