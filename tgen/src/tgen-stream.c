...
static gboolean _tgenstream_readAuthenticate(TGenStream* stream) {
    ...
    //tgen_message("[T] Stream with mmodel seed %"G_GUINT32_FORMAT" read %d bytes from network (authenticate)", stream->mmodel->prngSeed, bytes);
...
static gboolean _tgenstream_readModel(TGenStream* stream) {
    ...
    // tgen_message("[T] Stream with mmodel seed %"G_GUINT32_FORMAT" read %d bytes from network (model)", stream->mmodel->prngSeed, bytes);
...
static gboolean _tgenstream_readPayload(TGenStream* stream) {
    ...
    // tgen_message("[T] Stream with mmodel seed %"G_GUINT32_FORMAT" read %d bytes from network (payload)", stream->mmodel->prngSeed, bytes);
...
static void _tgenstream_onReadable(TGenStream* stream) {
    ...
    tgen_message("[T] active stream %s with markov model seed %"G_GUINT32_FORMAT" read %"G_GSIZE_FORMAT" more bytes",
            _tgenstream_toString(stream), stream->mmodel->prngSeed, totalBytes);
...
static gsize _tgenstream_flushOut(TGenStream* stream) {
    ...
    if(bytes < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
        ...
        tgen_message("[T] Stream with mmodel seed %"G_GUINT32_FORMAT" wrote %d bytes to network", stream->mmodel->prngSeed, bytes);
...
static gboolean _tgenstream_writePayload(TGenStream* stream) {
    ...
    if(newBufLen > 0) {
        ...
        tgen_message("[T] Wrote %d bytes of payload to network", bytes);
...
static void _tgenstream_onWritable(TGenStream* stream) {
    ...
    /**
    tgen_message("[T] active stream %s with markov model seed %"G_GUINT32_FORMAT" wrote %"G_GSIZE_FORMAT" more bytes",
            _tgenstream_toString(stream), stream->mmodel->prngSeed, totalBytes);
    */
