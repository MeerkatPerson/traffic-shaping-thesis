...
static TGenMarkovModel* _tgengenerator_createMarkovModel(
    ...
    if(seedGeneratorOpt->isSet) {
       ...
       //tgen_info("Seedgenerator seed adopted from start action %d", seedGeneratorOpt->value);

       GRand* prng = tgenpool_getRandom(seedGeneratorOpt->value);
       g_assert(prng);
       seed = g_rand_int(prng);

       tgen_info("Seeded markov model with seed %"G_GUINT32_FORMAT" found as property of start or flow action", seed);
    } else {
        /* just use the GLib-global prng */
       seed = g_random_int();
       tgen_info("Seeded markov model with RANDOM seed %"G_GUINT32_FORMAT" from GLib", seed);
    }

    if(modelPathOpt->isSet) {
       gchar* path = modelPathOpt->value;
       gchar* name = g_path_get_basename(path);
       mmodel = tgenmarkovmodel_newFromPath(name, seed, path); // TODO change back to non-static seed!!!
       g_free(name);
       ...
       mmodel = tgenmarkovmodel_newFromString(internalName, seed, graphmlBuffer); // TODO change back to non-static seed!!!
...
static void _tgengenerator_onCompleteHelper(TGenGenerator* gen, TGenNotifyFlags flags) {
    ...
    if((gen->hasError || gen->reachedEndState) && gen->numCompleted >= gen->numGenerated) {
        ...
        tgen_message("Generator status for action '%s' with mmseed %"G_GUINT32_FORMAT": completed %u of %u %ss, %s is complete",
                gen->actionIDStr, gen->mmodel->prngSeed, gen->numCompleted, gen->numGenerated, childType, parentType);
...
static gboolean _tgengenerator_createFlow(TGenGenerator* gen) {
    ...
    if(flow) {
        ...
        /*log association between parent flow generator and created flow based on their markov model seeds */
        tgen_message("[T] FLOW with mmodel seed %"G_GUINT32_FORMAT" was successfully generated by flowgenerator with mmodel seed %"G_GUINT32_FORMAT" ", flow->mmodel->prngSeed, gen->mmodel->prngSeed);
...
static gboolean _tgengenerator_createStream(TGenGenerator* gen) {
    ...
    /* log association between parent flow and created stream based on their mmodel seeds */
    tgen_message("[T] STREAM with mmodel seed %"G_GUINT32_FORMAT" was successfully generated by flow with mmodel seed %"G_GUINT32_FORMAT" ", stream->mmodel->prngSeed, gen->mmodel->prngSeed);
...
static void _tgengenerator_generateNext(TGenGenerator* gen) {
    ...
    if(obs == OBSERVATION_TO_ORIGIN || obs == OBSERVATION_TO_SERVER) {
        ...
        if(success) {
            tgen_message("Generated new %s successfully on %s action '%s', parent model has seed %"G_GUINT64_FORMAT" ",
                    childType, parentType, gen->actionIDStr, gen->mmodel->prngSeed);
        } else {
